...
export function registerBuiltinSpecs() {
    ...
    schema: [
      {
        ...
        help: 'Target URL, supports variable templates {var}',
        default: '',
      },
    ],
    defaults: { url: '' },
  ...
  registerNodeSpec({
    type: STEP_TYPES.KEY,
    version: 1,
    display: { label: 'Key', iconClass: 'icon-key', category: 'Actions' },
    ports: { inputs: 1, outputs: [{ label: 'default' }] },
    schema: [
      {
        key: 'keys',
  ...
  registerNodeSpec({
    ...
    schema: [
      ...
      { key: 'subflowId', label: 'Subflow ID', type: 'string', required: true },
      {
        ...
        type: 'number',
        default: 1,
        help: 'Run subflows concurrently (shallow-copies variables; results are not auto-merged)',
      },
    ],
