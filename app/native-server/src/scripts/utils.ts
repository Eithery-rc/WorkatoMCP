  ...
  try {
    ...
    for (const browserType of browsersToRegister) {
      ...
      try {
        ...
        // Windows requires an additional registry entry pointing to the manifest path.
...
// Load is-admin only on Windows; on other platforms admin checks use process.getuid().
let isAdmin: () => boolean = () => false;
if (process.platform === 'win32') {
  try {
    isAdmin = require('is-admin');
  } catch (error) {
    console.warn('is-admin package missing; Windows admin detection unavailable');
    console.warn(error);
  }
}
...
export async function registerWithElevatedPermissions(): Promise<void> {
  try {
    ...
    // Windows: set system-level registry entry
    if (os.platform() === 'win32') {
