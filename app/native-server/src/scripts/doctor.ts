...
export async function collectDoctorReport(options: DoctorOptions): Promise<DoctorReport> {
  ...
  const packageName = typeof pkg.name === 'string' ? pkg.name : 'workatomcp-bridge';
  ...
  for (const browser of browsersToCheck) {
    ...
    if (
      ...
      !expectedOrigins.some((o) => allowedOrigins.includes(o))
    ) {
      issues.push(`allowed_origins missing any of: ${expectedOrigins.join(', ')}`);
    }
