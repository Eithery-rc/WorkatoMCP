...
3. Register the Native Messaging host:
   ...
   # System-level (if user-level fails)
...
This application is started automatically by the Chrome extension.
`;
...
filesToMakeExecutable.forEach((file) => {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      console.log(`Set execute permissions (755) for ${file}`);
    } else {
      console.warn(`Warning: ${filePath} not found, skipping chmod`);
    }
  } catch (error) {
    console.error(`Error setting execute permissions for ${file}:`, error);
  }
});
