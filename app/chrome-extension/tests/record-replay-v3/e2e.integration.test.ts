 ...
 * - Service-level RPC (calls internal handler directly, no Port mock)
 */
...
/**
 * Extract the list of event types for a given runId.
 */
...
describe('V3 service-level E2E', () => {
  ...
  describe('Event streaming', () => {
    ...
    it('subscribe with runId filter only receives events for that run', async () => {
      ...
      // Enqueue run1 first and wait for it to complete
      const { runId: runId1 } = await client.call<{ runId: string }>('rr_v3.enqueueRun', {
        flowId: flow1.id,
      });
      await h.waitForTerminal(runId1);

      // Subscribe to only runId1 events (but run1 is already done)
  ...
  describe('Recovery', () => {
    it('orphan running lease is requeued and run can complete', async () => {
      // Stop current harness and create a new one without auto-starting scheduler
      await h.dispose();
      ...
      // Create an orphan queue item (held by old owner)
      ...
      // Start scheduler — run should be able to complete
      h.scheduler.start();
    ...
    it('adopts orphan paused items', async () => {
      ...
      // Queue item should still be paused, but with the new owner
    ...
    it('cleans terminal runs left in queue', async () => {
      ...
      // Simulate crash: run completed but queue item was not cleaned up
      await h.storage.queue.enqueue({ id: runId, flowId: flow.id, priority: 0 });
      await h.storage.queue.markRunning(runId, 'owner-old', Date.now());
