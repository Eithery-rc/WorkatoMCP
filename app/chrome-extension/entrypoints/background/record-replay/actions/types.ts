 ...
 * Core type definitions for enterprise-grade record-replay.
 */
...
export interface TimeoutPolicy {
  ms: Milliseconds;
  /** 'attempt' = each attempt timed independently; 'action' = whole action timed together */
...
export interface RetryPolicy {
  /** Number of retries (excludes the initial attempt) */
  retries: number;
  /** Delay between retries */
  intervalMs: Milliseconds;
  /** Backoff strategy */
  backoff?: BackoffKind;
  /** Maximum interval (used for exp/linear backoff) */
...
/**
 * ActionParamsByType uses interface declaration to allow external modules
 * to extend Action types via declaration merging (OCP).
 */
...
/**
 * Action output type map (extendable via declaration merging).
 */
...
export interface ActionExecutionResult<T extends ActionType = ActionType> {
  ...
  error?: ActionError;
  /** Next edge label (for conditional branches) */
  nextLabel?: EdgeLabel;
...
export interface ActionHandler<T extends ExecutableActionType = ExecutableActionType> {
  ...
  /** Generate a human-readable description (for UI display) */
...
export interface Flow {
  ...
  /** Sub-flows (used by foreach/while/loopElements) */
  subflows?: Record<
    SubflowId,
