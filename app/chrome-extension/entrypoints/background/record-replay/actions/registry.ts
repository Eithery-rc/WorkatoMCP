/**
 * Action Registry — handler registration and execution pipeline.
 *
 * Features:
 * - Dynamic handler registration / deregistration
 * - Middleware hooks (beforeExecute, afterExecute)
...
export class ActionRegistry {
  ...
  /**
   * Register a beforeExecute hook. Returns a disposer.
   */
  ...
  /**
   * Register an afterExecute hook. Returns a disposer.
   */
  ...
  /**
   * Register a set of hooks in one call. Returns a disposer.
   */
  ...
  /**
   * Validate an action's configuration against its handler.
   */
  ...
  /**
   * Execute an action, applying retry/timeout policies and all registered hooks.
   */
  async execute<T extends ExecutableActionType>(
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      // afterExecute hooks (may replace result)
      for (const hook of this.afterHooks) {
        try {
