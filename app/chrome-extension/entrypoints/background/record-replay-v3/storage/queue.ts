/**
 * @fileoverview RunQueue persistence — CRUD operations and atomic claim for the run queue.
 */
...
/**
 * Create a RunQueue persistence implementation (atomic claim, Phase 3).
 */
  ...
  return {
    ...
    async recoverOrphanLeases(
      ...
      return withTransaction(RR_V3_STORES.QUEUE, 'readwrite', async (stores) => {
        ...
        /**
         * Scan and requeue orphaned running items.
         * Orphan: no lease, or lease.ownerId !== currentOwnerId.
         * Strategy: status -> queued, drop lease, preserve attempt.
         */
        const recoverRunningItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Requeue: drop lease, set status to queued
              const { lease: _droppedLease, ...itemWithoutLease } = item;
              const updated: RunQueueItem = {
                ...itemWithoutLease,
                status: 'queued',
         ...
         * Orphan: no lease, or lease.ownerId !== currentOwnerId.
         * Strategy: keep status=paused, update lease.ownerId to new ownerId, renew TTL.
         */
        const recoverPausedItems = (): Promise<void> =>
          new Promise<void>((resolve, reject) => {
            ...
            request.onsuccess = () => {
              ...
              // Adopt: update lease to new ownerId and renew TTL
              const updated: RunQueueItem = {
                ...item,
                updatedAt: now,
                lease: {
        ...
        // Sequential: process running items first, then paused
