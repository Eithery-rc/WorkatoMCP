/**
 * @fileoverview TriggerManager — manages the lifecycle of all trigger handlers.
 *
 * Responsibilities:
 * - Load triggers from TriggerStore and install them.
 * - Handle trigger fire events and call enqueueRun.
 * - Provide storm-protection (cooldown per-trigger + maxQueued global best-effort).
 *
 * Design:
 * - Orchestrator pattern: TriggerManager delegates per-kind logic to individual handlers.
 * - Handler factory pattern: handlers are created at construction time with an injected fireCallback.
 * - Storm protection: per-trigger cooldown + global maxQueued (best-effort, not atomic).
 */
...
export interface TriggerManagerStormControl {
   ...
   * New fires are rejected when this limit is reached.
   * undefined disables the check.
   * Note: this is a best-effort check — it is not atomic.
   */
  maxQueued?: number;
}
...
export interface TriggerManager {
  ...
  stop(): Promise<void>;
  /** Refresh triggers — reload from storage and reinstall. */
  refresh(): Promise<void>;
  /**
   * Manually fire a trigger.
   * Intended for RPC/UI calls on manual triggers.
   */
  fire(
    triggerId: TriggerId,
...
export function createTriggerManager(deps: TriggerManagerDeps): TriggerManager {
  ...
  // Fire callback injected into each handler
  const fireCallback: TriggerFireCallback = {
    onFire: async (triggerId, context) => {
      // Catch all exceptions to avoid propagating into chrome API listeners
      try {
  ...
  // Instantiate handlers from factories
  ...
  /**
   * Handle a trigger fire (internal).
   * @param throwOnDrop If true, throw instead of silently dropping on cooldown/maxQueued.
   * @returns EnqueueRunResult or null (silent drop).
   */
  async function handleFire(
    ...
    // Checked before setting cooldown to avoid incorrectly marking the trigger as fired.
    ...
    // Record lastFireAt to suppress concurrent fires (set after maxQueued check passes)
    if (cooldownMs > 0) {
    ...
    try {
      ...
      // Roll back cooldown timestamp on enqueue failure
      if (cooldownMs > 0) {
   ...
   * Throws rather than silently dropping — used for RPC/UI calls.
   */
  async function fire(
    triggerId: TriggerId,
    context: { sourceTabId?: number; sourceUrl?: string } = {},
  ...
  async function doRefresh(): Promise<void> {
    ...
    // Uninstall all then reinstall (simple strategy for consistency).
    // Best-effort: a single handler failure does not affect others.
  ...
  async function stop(): Promise<void> {
    ...
    if (refreshPromise) {
      try {
        ...
        // Ignore refresh errors during stop
      }
    }
