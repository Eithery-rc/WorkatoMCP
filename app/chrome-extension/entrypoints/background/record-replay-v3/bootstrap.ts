 ...
 * - The global keepalive-manager singleton is used to avoid multiple controllers conflicting
 * - RunExecutor delegates to RunRunner for actual flow execution
 */
...
/**
 * Handle returned by bootstrapV3, giving access to all V3 subsystems.
 */
...
/**
 * Resolve the tab ID needed to run a flow.
 ...
 * Re-reads the latest RunRecord to get the correct startedAt value.
 */
...
/**
 * Create the default RunExecutor, which delegates to RunRunner for flow execution.
 */
function createDefaultRunExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    // 4. Sync attempt count to RunRecord
    try {
      await deps.storage.runs.patch(runId, {
    ...
    try {
      ...
      // Register with RunnerRegistry for DebugController and RPC access
      ...
      // 7. Close ephemeral tab if we created it
      if (shouldClose) {
...
/**
 * Return the current runtime handle, or null if not started.
 */
...
/**
 * Whether the V3 runtime has been started.
 */
